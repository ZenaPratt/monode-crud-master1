...
app.get('/', function(req, res) {
  res.render('home',{home:"active",session: req.session
  });
});
...
app.get('/', function(req, res){
    ...
    var Client = new Db('teen-xone', new Server('127.0.0.1', 27017, {}));
    Client.open(function(err, pClient) {
        Client.collection('artists', listData);
        //Client.close();
    });
...
app.get('/add_record', function(req, res){
    res.render('add.html', { layout : false , 'title' : 'Teen Xone'});
})
...
app.post('/save_record', function(req, res){
    ...
    var Client = new Db('teen-xone', new Server('127.0.0.1', 27017, {}));
    Client.open(function(err, pClient) {
        Client.collection('artists', insertData);
        Client.close();
    });    
...
app.get('/edit_record/:id', function(req, res){
    ...
    var listData = function(err, collection) {
        ...
        collection.findOne({'_id' : chosenId} , function(err, results) {
            console.log(results);
            res.render('edit.html', { layout : false , 'title' : 'Teen Xone', 'results' : results });
        });
    }
...
app.post('/update_record', function(req, res){
    ...
    var Client = new Db('teen-xone', new Server('127.0.0.1', 27017, {}));
    Client.open(function(err, pClient) {
        Client.collection('artists', updateData);
        Client.close();
    });    
...
app.get('/delete_record/:id', function(req, res){
    ...
    var Client = new Db('teen-xone', new Server('127.0.0.1', 27017, {}));
    Client.open(function(err, pClient) {
        Client.collection('artists', removeData);
        //Client.close();
    });  
    res.redirect('/');
});
